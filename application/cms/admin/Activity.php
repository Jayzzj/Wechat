...
use app\cms\model\Activity as ActivityModel;
...
class Activity extends Admin
{
    ...
    public function index()
    {
        ...
        // 数据列表
        $data_list = ActivityModel::where($map)->order($order)->paginate();
...
//            }
            ...
            if ($activity = ActivityModel::create($data)) {
                // 记录行为
                action_log('activity_add', 'cms_activity', $activity['id'], UID, $data['title']);
...
//                ['radio', 'status', '立即启用', '', ['否', '是']]
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $activity_title = ActivityModel::where('id', 'in', $ids)->column('title');
        return parent::setStatus($type, ['activity_'.$type, 'cms_activity', 0, UID, implode('、', $activity_title)]);
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        $shop  = ActivityModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $shop . ')，新值：(' . $value . ')';
        return parent::quickEdit(['activity_edit', 'cms_activity', $id, UID, $details]);
    }
}