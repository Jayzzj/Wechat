...
//            }
            ...
            if ($notice = ServiceModel::create($data)) {
                // 记录行为
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (ServiceModel::update($data)) {
                // 记录行为
                action_log('notice_edit', 'cms_notice', $id, UID, $data['title']);
...
//                ['radio', 'status', '立即启用', '', ['否', '是']]
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $advert_name = ServiceModel::where('id', 'in', $ids)->column('title');
        return parent::setStatus($type, ['notice_'.$type, 'cms_notice', 0, UID, implode('、', $advert_name)]);
    }
